...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_SERVER_URL}/coachinfo/username`, {
          headers: {
        ...
        setAge(data.age);
        setGender(data.sex);
        setCoachingPeriod(data.career);
        setCoachingCertificate(data.normalLicense);
        setPersonal(data.cprLicense);
        setOther(data.etcLicense);
        setCoaching(data.sportsLicense);
      } catch (error) {
  ...
  const complete = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`${import.meta.env.VITE_SERVER_URL}/coachinfo`,
        {
          age,
          sex: gender,
    ...
    <Back/>
      <div className={styles.container}>
        <form className={styles.form} onSubmit={complete}>
          ...
          <button type="submit" className={styles.Cbutton}>등록 완료</button>
        </form>
      </div>
      {showModal && (
